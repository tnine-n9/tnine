...
if not success then warn("Failed to process " .. tostring(url) .. ": " .. tostring(result)) end
...
local settingsTable = {
General = {rayfieldOpen = {Type = 'bind', Value = 'K', Name = 'Rayfield Keybind'}},
System = {usageAnalytics = {Type = 'toggle', Value = true, Name = 'Anonymised Analytics'}}
}
...
if not success then if writefile then warn('Rayfield had an issue accessing configuration saving capability.') end end
end
...
if useStudio then print('Sending Analytics')
else
...
elseif not cachedSettings then sendReport("execution", "Rayfield") end
end
...
local UI_WIDTH = 700
local UI_HEIGHT = 500
local MOBILE_WIDTH = 450
local MOBILE_HEIGHT = 600
local TABLIST_WIDTH = 150
local MOBILE_TABLIST_WIDTH = 100
...
if debugX then warn(HttpService:JSONEncode(Data)) end
...
TweenService:Create(Main, TweenInfo.new(0.5, Enum.EasingStyle.Exponential), {Size = UDim2.new(0, useMobileSizing and MOBILE_WIDTH or UI_WIDTH, 0, 0)}):Play()
TweenService:Create(Main.Topbar, TweenInfo.new(0.5, Enum.EasingStyle.Exponential), {Size = UDim2.new(0, useMobileSizing and MOBILE_WIDTH or UI_WIDTH, 0, 45)}):Play()
...
TweenService:Create(Main, TweenInfo.new(0.5, Enum.EasingStyle.Exponential), {Size = useMobileSizing and UDim2.new(0, MOBILE_WIDTH, 0, MOBILE_HEIGHT) or UDim2.new(0, UI_WIDTH, 0, UI_HEIGHT)}):Play()
TweenService:Create(Topbar, TweenInfo.new(0.5, Enum.EasingStyle.Exponential), {Size = UDim2.new(0, useMobileSizing and MOBILE_WIDTH or UI_WIDTH, 0, 45)}):Play()
...
TweenService:Create(Main, TweenInfo.new(0.5, Enum.EasingStyle.Exponential), {Size = UDim2.new(0, useMobileSizing and MOBILE_WIDTH or UI_WIDTH, 0, 45)}):Play()
TweenService:Create(Topbar, TweenInfo.new(0.5, Enum.EasingStyle.Exponential), {Size = UDim2.new(0, useMobileSizing and MOBILE_WIDTH or UI_WIDTH, 0, 45)}):Play()
task.wait(0.3)
...
local Window = {}
TabList.Position = UDim2.new(0, 0, 0, TOPBAR_HEIGHT)
TabList.Size = UDim2.new(0, useMobileSizing and MOBILE_TABLIST_WIDTH or TABLIST_WIDTH, 1, -TOPBAR_HEIGHT)
local oldTabList = TabList
local newTabList = Instance.new("ScrollingFrame")
newTabList.Name = oldTabList.Name
newTabList.Position = oldTabList.Position
newTabList.Size = oldTabList.Size
newTabList.BackgroundColor3 = oldTabList.BackgroundColor3
newTabList.BackgroundTransparency = oldTabList.BackgroundTransparency
newTabList.BorderSizePixel = 0
newTabList.ScrollBarThickness = 6
newTabList.ScrollBarImageColor3 = SelectedTheme.TextColor
newTabList.ScrollBarImageTransparency = 0.7
newTabList.CanvasSize = UDim2.new(0, 0, 0, 0)
newTabList.AutomaticCanvasSize = Enum.AutomaticSize.Y
newTabList.Parent = Main
for _, child in ipairs(oldTabList:GetChildren()) do
child.Parent = newTabList
end
oldTabList:Destroy()
TabList = newTabList
...
local templateBtn = TabList:FindFirstChild("Template")
if templateBtn and templateBtn:IsA("Frame") then
templateBtn.Size = UDim2.new(1, -10, 0, 30)
if templateBtn:FindFirstChild("Title") then
templateBtn.Title.Position = UDim2.new(0, 35, 0.5, 0)
templateBtn.Title.Size = UDim2.new(1, -40, 0, 14)
end
if templateBtn:FindFirstChild("Image") then
templateBtn.Image.Position = UDim2.new(0, 8, 0.5, 0)
end
end
for _, tabBtn in ipairs(TabList:GetChildren()) do
if tabBtn:IsA("Frame") and tabBtn.Name ~= "Template" and tabBtn.Name ~= "Placeholder" then
tabBtn.Size = UDim2.new(1, -10, 0, 30)
if tabBtn:FindFirstChild("Title") then
tabBtn.Title.Position = UDim2.new(0, 35, 0.5, 0)
tabBtn.Title.Size = UDim2.new(1, -40, 0, 14)
end
if tabBtn:FindFirstChild("Image") then
tabBtn.Image.Position = UDim2.new(0, 8, 0.5, 0)
end
end
end
Elements.Position = UDim2.new(0, useMobileSizing and MOBILE_TABLIST_WIDTH or TABLIST_WIDTH, 0, TOPBAR_HEIGHT)
Elements.Size = UDim2.new(1, -(useMobileSizing and MOBILE_TABLIST_WIDTH or TABLIST_WIDTH), 1, -TOPBAR_HEIGHT)
...
TabButton.Title.Position = UDim2.new(0, 35, 0.5, 0)
...
task.wait(0.1)
TweenService:Create(Main, TweenInfo.new(0.6, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out), {Size = useMobileSizing and UDim2.new(0, MOBILE_WIDTH, 0, MOBILE_HEIGHT) or UDim2.new(0, UI_WIDTH, 0, UI_HEIGHT)}):Play()
TweenService:Create(Main.Shadow.Image, TweenInfo.new(0.5, Enum.EasingStyle.Exponential), {ImageTransparency = 0.6}):Play()
